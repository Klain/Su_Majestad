...
event("rats_granary", "Granero con ratas", "Las reservas del norte han sido infestadas.", [["Quemad el granero", { food: -8, threat: -3, people: -2 }, { addTags: ["decisive_ruler"] }], ["Contratad cazadores", { gold: -6, food: -3, people: 2 }], ["Ignoradlo", { food: -10 }, { defer: [{ delay: 4, eventId: "rats_return" }] }]]),
  ...
  event("golden_harvest", "Cosecha dorada", "Los campos producen más de lo esperado.", [["Almacenar", { food: 10 }, { successChance: 85, resultText: "El grano se conserva bien." }], ["Celebrar feria", { food: 5, gold: 4, people: 3 }, { addTags: ["held_fair"] }], ["Diezmo especial", { food: 3, faith: 4, people: -2 }, { addTags: ["helped_church"] }]], { probabilistic: true, failureResult: { immediate: { food: -5 }, resultText: "Una plaga arruina la apuesta y se ejecuta el fallo común del evento." } }),
  ...
  event("empty_granaries", "Graneros vacíos", "Los panaderos advierten que la harina no alcanza para otra semana.", [["Comprar grano caro", { gold: -8, food: 8 }], ["Racionar el pan", { food: 4, people: -4 }, { addTags: ["rationed_bread"] }]], { family: "seneschal", weight: 0.8, resourceConditions: { food: { max: 30 } } }),
  event("royal_auditors", "Auditores reales", "Los escribanos de palacio revisan cuentas y privilegios con celo renovado.", [["Revisad los libros", { gold: 4, crown: -2 }], ["Mostrad clemencia", { crown: 2, gold: -3 }]], { family: "chancellor", weight: 0.6, resourceWeights: [{ resource: "crown", min: 70, multiplier: 2 }] }),
...
event("forge_cold_coal", "Carbón húmedo", "Tomás de la Fragua llega con las manos negras y la voz seca: sin carbón seco, la forja se apagará justo cuando más encargos hay.", [
  ...
  event("merchant_loan_offer", "Préstamo del gremio", "El Gremio de Mercaderes ofrece un préstamo sin preguntas. Lo preocupante no es el oro: es la sonrisa.", [
    ...
    ["Negociar interés menor", { gold: 7 }, { successChance: 55, characters: [{ id: "gremio_mercaderes" }], addTags: ["merchant_loan_negotiated"], resultText: "El gremio acepta menos interés, sorprendido por tu firmeza." }],
  ...
  ], { family: "merchants", weight: 0.9, probabilistic: true, failureResult: { immediate: { gold: 3, nobility: -1 }, resultText: "El gremio reduce la oferta y difunde que la corona regatea como mendiga." } }),
...
event("garrison_boots", "Botas rotas", "El Capitán Rodrigo informa que media guarnición patrulla con botas abiertas. Un soldado cojo defiende peor la muralla.", [
  ...
  event("blackmail_letter", "Carta de chantaje", "El Cuervo Gris ofrece una carta que compromete a un consejero. Su precio es alto porque el miedo siempre lo es.", [
    ...
    ["Tender trampa al Cuervo", { threat: -4 }, { successChance: 45, characters: [{ id: "cuervo_gris" }], addTags: ["tried_trapping_crow"], resultText: "La trampa captura a un mensajero del Cuervo, no al Cuervo." }],
  ...
  ], { family: "spy", weight: 0.85, probabilistic: true, failureResult: { immediate: { threat: 3, nobility: -1 }, resultText: "El Cuervo olía la trampa antes de que existiera." } }),
...
event("southern_fever", "Fiebre del sur", "El doctor Avelino informa de una fiebre leve en el barrio de curtidores. Leve es una palabra que cambia deprisa.", [
  ...
  event("frozen_road", "Camino helado", "El camino norte se ha vuelto una cinta de hielo. Los carros se retrasan y los juramentos llegan antes que la comida.", [
    ...
    ["Permitir paso bajo riesgo", { food: 4, gold: 2 }, { successChance: 55, addTags: ["risked_icy_road"], resultText: "Los carros cruzan con ruedas lentas y plegarias rápidas." }]
  ], { family: "merchants", weight: 0.85, probabilistic: true, failureResult: { immediate: { food: -4, people: -2 }, resultText: "Un carro cae y el hielo cobra peaje." } }),
