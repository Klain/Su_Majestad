...
function validateDeveloperEvents(items) { const errors = [], warnings = [], ids = new Set(), duplicateIds = new Set(); const actorIds = new Set((actors || []).map((actor) => actor.id)); const familyIds = new Set((families || []).map((family) => family.id)); items.forEach((item) => { if (!item.id) return; if (ids.has(item.id)) duplicateIds.add(item.id); else ids.add(item.id); }); items.forEach((item, index) => { if (!item.id) errors.push(`#${index}: evento sin id.`); if (!item.title) errors.push(`${item.id || index}: evento sin title.`); if (!item.text) errors.push(`${item.id || index}: evento sin text.`); if (!Array.isArray(item.options) || !item.options.length) errors.push(`${item.id || index}: evento sin options.`); if (item.requiresTags && !Array.isArray(item.requiresTags)) errors.push(`${item.id}: requiresTags debe ser array.`); if (item.forbiddenTags && !Array.isArray(item.forbiddenTags)) errors.push(`${item.id}: forbiddenTags debe ser array.`); if (item.family && !familyIds.has(item.family)) warnings.push(`${item.id}: family primaria '${item.family}' no existe en families.`); validateResourceMap(item.resourceConditions, `${item.id}: resourceConditions`, errors); (item.resourceWeights || []).forEach((rule, ruleIndex) => validateResourceRule(rule, `${item.id}: resourceWeights ${ruleIndex + 1}`, errors)); (item.options || []).forEach((option, optionIndex) => validateDeveloperOption(item, option, optionIndex, ids, actorIds, errors, warnings)); }); duplicateIds.forEach((id) => errors.push(`ID duplicado: ${id}.`)); return { errors, warnings }; }
function validateDeveloperOption(item, option, optionIndex, ids, actorIds, errors, warnings) { if (!option.label) errors.push(`${item.id}: opción ${optionIndex + 1} sin label.`); [option.immediate, option.effects, ...(option.outcomes || []).map((outcome) => outcome.immediate || outcome.effects)].filter(Boolean).forEach((effects) => validateResourceMap(effects, `${item.id}: opción ${optionIndex + 1}`, errors)); (option.characters || []).forEach((character) => { if (character.id && !actorIds.has(character.id)) warnings.push(`${item.id}: actor '${character.id}' no existe.`); }); normalizeDeferList(option.defer).forEach((defer) => { if (!Number.isFinite(Number(defer.delay)) || Number(defer.delay) < 1) errors.push(`${item.id}: defer.delay debe ser >= 1.`); if (defer.eventId && !ids.has(defer.eventId)) errors.push(`${item.id}: defer.eventId inexistente '${defer.eventId}'.`); (defer.branches || []).forEach((branch) => { if (branch.eventId && !ids.has(branch.eventId)) errors.push(`${item.id}: defer.branches.eventId inexistente '${branch.eventId}'.`); }); }); }
function validateResourceMap(effects, label, errors) { if (!effects || typeof effects !== "object") return; Object.keys(effects).forEach((key) => { if (!RESOURCE_KEYS.has(key)) errors.push(`${label}: recurso inválido '${key}'.`); }); }
function validateResourceRule(rule, label, errors) { if (!rule || typeof rule !== "object") return; if (rule.resource && !RESOURCE_KEYS.has(rule.resource)) errors.push(`${label}: recurso inválido '${rule.resource}'.`); }
